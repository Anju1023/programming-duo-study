...
interface UnitSectionProps {
  unit: any; // Type this properly with Database types later
  activeLessonId?: number;
  completedLessonIds: number[];
}

export const UnitSection = ({ unit, activeLessonId, completedLessonIds }: UnitSectionProps) => {
  return (
      ...
      <div className="flex flex-col gap-8 relative items-center">
        {unit.lessons.map((lesson: any, index: number) => {
          ...
          // OR if it's undefined, we find the first one not in completed.
           ...
           if (isCompleted) {
               status = "COMPLETED";
           } else if (lesson.id === activeLessonId || (!activeLessonId && index === 0 && completedLessonIds.length === 0)) {
               status = "ACTIVE";
           } else {
               ...
               // If previous one is COMPLETED, this one is ACTIVE (if not completed).
                ...
                const prevCompleted = prevLesson ? completedLessonIds.includes(prevLesson.id) : true; // First lesson has no prev, so "true"
