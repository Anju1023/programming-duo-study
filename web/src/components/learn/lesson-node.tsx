...
export const LessonNode = ({ id, index, totalCount, status }: LessonNodeProps) => {
    ...
    <div 
        ...
        style={{
            ...
            // In a real app we might use a more robust grid system, but this mimics the "path" look
             ...
             transform: cycleIndex > 4 ? `translateX(-${indentation.replace("-", "")})` : `translateX(${indentation})`
        }}
    >
      <Link
        ...
        className={cn(
          ...
          status === "ACTIVE" && "bg-primary border-primary-foreground/50 text-primary-foreground shadow-[0_0_20px_rgba(59,130,246,0.5)]", // Glow effect for active
          status === "LOCKED" && "bg-muted border-muted-foreground/30 text-muted-foreground pointer-events-none"
        )}
