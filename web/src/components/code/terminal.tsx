...
export function Terminal({ className }: TerminalProps) {
	...
	return (
		<div
			className={cn(
				'flex flex-col overflow-hidden rounded-lg border bg-slate-950 text-slate-50 font-mono text-sm shadow-xl',
				className
			)}
		>
			...
			<ScrollArea className="flex-1 p-4 h-[200px] w-full">
				...
				{stdout.map((line, i) => (
					<div key={`out-${i}`} className="whitespace-pre-wrap">
						{line}
					</div>
				))}
				{stderr.map((line, i) => (
					<div key={`err-${i}`} className="whitespace-pre-wrap text-red-400">
						{line}
					</div>
				))}
				{isLoading && (
